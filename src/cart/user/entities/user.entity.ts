...
export class User {
  ...
  @OneToMany(() => CartSummary, cartSummary => cartSummary.shopKeepId)
  shopKeeps: CartSummary[];

  @OneToMany(() => CartSummary, cartSummary => cartSummary.customerId)
  customers: CartSummary[];

  // @BeforeRemove()
  // async checkForSales() {
  //   const cartSummaryService = new CartSummaryService();
  //   const sales = await cartSummaryService.findSalesByUserId(this.user_id);
  //   if (sales.length > 0) {
